...
public class DausController {
	...
	public ResponseEntity<Jugador> creaJugador(@RequestBody Jugador jugador) {
		...
		//return ResponseEntity.ok().body(serviceJugador.saveJugador(jugador));//ok().(service.saveJugador(jugador));
	}
	...
	public ResponseEntity<Jugador> updateJugadorNom(@PathVariable(value="id_jugador")int id_Jugador
			,@RequestBody String nom){
		Jugador jugador=serviceJugador.updateJugadorNom(id_Jugador, nom);
		return ResponseEntity.ok().body(jugador);	//return new ResponseEntity<Jugador>(jugador, HttpStatus.OK)	
	}
