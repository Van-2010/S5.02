...
public class JugadorServiceImp implements IJugadorService {
	...
	// 3.MODIFICA EL NOM D'UN JUGADOR
	/*
	 * public Jugador updateJugadorNom(int id,String nom,Jugador jugador)
	 * {//modifica nom jugadors
	 * if(jugadorDao.findById(id)==null||jugadorDao.existsByNom(jugador.getNom())){
	 * System.out.println("El jugador amb id "+id+" no existeix"); }else {
	 * jugador.setNom(nom); } return jugadorDao.save(jugador);
	 * 
	 * }
	 */
	...
	public Map<String, Double> getLlistaPercentatgeMigJugadors() {// llista tots jugadors i el seu percentage mig
		...
		if (jugadors != null && jugadors.size() > 0) { // si llista jugadors no es null i més gran que 0
			...
			for (Jugador jugador : jugadors) { // recorro llista jugagdors
				tiradesActualJugador = tiradesDao.getLlistaTiradesByJugadorIdJugador(jugador.getIdJugador());
